...
export const getbyid= (req, res) => {
    ...
    let qr = `SELECT * FROM access WHERE id=${pID}`
    data.query(qr, (err, result) => {
        if (!err) {
            res.send(result)
        }
...
export const postmethod = (req, res) => {
    ...
    let qr = `INSERT  INTO access ( Name, price, description, pics) VALUES(?)`
    const values = [
        req.body.Name,
        req.body.price,
...
export const updatemethod = (req, res) => {
    ...
    let qr = `UPDATE  access  SET  Name='${name}', price='${price}', description='${desc}', pics='${img} ' WHERE id=${pID}`
...
export const deletemethod =(req, res) => {
    let pID = req.params.id
    let qr = `DELETE FROM access WHERE id=${pID}`
    data.query(qr, (err, result) => {
        if (!err) {
            res.send(result)
        } else {
